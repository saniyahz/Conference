...
export default function LoadingSpinner({ message = 'Loading...', stage = 'story' }: LoadingSpinnerProps) {
  ...
  // Realistic time estimates (pipeline: plate → inpaint → score per page, 2 pages at a time)
  const estimatedTime = stage === 'story' ? '~30 seconds' : '~5-8 minutes'
      ...
      <div className="mt-6 bg-white rounded-xl p-4 shadow-lg border-2 border-purple-200 max-w-md w-full">
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center gap-2 text-purple-700">
            ...
            <span className="font-medium">
              {stage === 'story' ? 'Writing your story...' : 'Painting & checking illustrations...'}
            </span>
          </div>
        ...
        <div className="mt-3 h-2 bg-purple-100 rounded-full overflow-hidden">
          <div
            ...
            style={{
              // Story: ~30 seconds (3.3% per second), Images: ~6 minutes = 360s (0.26% per second)
              width: stage === 'story'
                ? `${Math.min(elapsedTime * 3.3, 95)}%`
                : `${Math.min(elapsedTime * 0.26, 95)}%`
            }}
          />
        </div>
