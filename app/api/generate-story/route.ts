...
function generateImagePrompts(story: any, originalPrompt: string): string[] {
  ...
  const char = concepts.characterName || concepts.character || 'friendly hero'
  const charType = concepts.character || 'character'
  ...
  const prompts = [
    ...
    `${artStyle}. A cute ${charType} named ${char} with bright eyes in a magical forest with sparkling trees, colorful flowers, and butterflies at sunrise.`,

    // Page 2: Adventure begins
    `${artStyle}. ${char} the ${charType} on a glowing golden path in an enchanted forest with owls, squirrels, butterflies, mystical trees, and rainbow flowers.`,

    // Page 3: Solving the problem together
    `${artStyle}. ${char} the ${charType} with animal friends (owls, squirrels, rabbits, butterflies) working together in a forest clearing with a magical glowing crystal or rainbow in the sky.`,

    // Page 4: Happy ending celebration
    `${artStyle}. ${char} the ${charType} celebrating with animal friends in a meadow at sunset with orange, pink, and gold sky, magical sparkles, and a distant castle.`,
  ]
