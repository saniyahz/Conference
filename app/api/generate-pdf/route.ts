...
async function getImageAsBase64(url: string, maxRetries = 5): Promise<string> {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = error
      console.error(`Image fetch attempt ${attempt + 1}/${maxRetries + 1} failed for ${url.substring(0, 100)}...:`, error.message)
    ...
    for (let i = 0; i < story.pages.length; i++) {
      ...
      if (imageUrl && typeof imageUrl === 'string' && imageUrl.trim() !== '') {
        try {
          ...
          console.error(`Failed to load image for page ${i + 1}:`, error.message)
